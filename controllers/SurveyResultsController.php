...
class SurveyResultsController extends Controller
{
    ...
    protected function downloadCsv(Survey $survey)
    {
        ...
        $csvFileName = preg_replace('/[^A-Za-z0-9_-ก-ฮ]/', '', str_replace(' ', '_', $survey->survey_name));
        ...
        foreach ($survey->questions as $question) {
            $headers[] = iconv('UTF-8', 'TIS-620', $question->question_text);
            //$headers[] = "Time";
        }
        ...
        foreach ($survey->responses as $response) {
            $row = [];
            $row[] = iconv('UTF-8', 'TIS-620',$response->time_taken);
            foreach ($survey->questions as $question) {
                $field = 'question_' . $question->question_id;
                $row[] = iconv('UTF-8', 'TIS-620',$response->$field);
                //$row[] = iconv('UTF-8', 'TIS-620',$response->time_taken);
